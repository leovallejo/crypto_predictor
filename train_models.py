...
def prepare_data(df):
    ...
    X = feature_scaler.fit_transform(df[feature_cols])
    y = close_scaler.fit_transform(df[['close']])
    
    # Create sequences
    X_seq, y_seq = [], []
    for i in range(SEQUENCE_LENGTH, len(df) - FORECAST_HORIZON):
        X_seq.append(X[i-SEQUENCE_LENGTH:i])
        y_seq.append(y[i:i+FORECAST_HORIZON].flatten())
...
def build_model(hp):
    model = Sequential()
    model.add(Conv1D(
        hp.Int('conv1_filters', 32, 256, step=32),
        hp.Int('conv1_kernel', 3, 9, step=2),
        activation='relu',
...
def train_model(token, timeframe):
    ...
    try:
        ...
        logger.error(f"Failed {tag}: {str(e)}")
        send_telegram_message(f"❌ {tag} failed: {str(e)}")

def main():
    send_telegram_message("🏁 Starting training session")
